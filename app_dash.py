...
# Professional logging configuration for QUANTUM CAPITAL
def setup_quantum_logging():
    """Setup comprehensive logging system for QUANTUM CAPITAL trading platform."""
    ...
    logger = logging.getLogger("QUANTUM_CAPITAL")
    logger.setLevel(logging.INFO)
    
    # Clear existing handlers
    logger.handlers.clear()
    
    # Console handler with professional format
    console_handler = logging.StreamHandler()
    console_handler.setLevel(logging.INFO)
    console_format = logging.Formatter(
        "🚀 [%(asctime)s] QUANTUM CAPITAL %(levelname)s: %(message)s",
        datefmt="%H:%M:%S"
    )
    ...
    error_file = os.path.join(log_dir, f"quantum_errors_{datetime.now().strftime('%Y%m%d')}.log")
...
# Initialize logger
QUANTUM_LOGGER = setup_quantum_logging()

# Enhanced monitoring functions
def log_ai_performance(function_name, success, duration_ms, details=""):
    """Log AI function performance metrics."""
    status = "SUCCESS" if success else "FAILURE"
    QUANTUM_LOGGER.info(f"AI_PERF | {function_name} | {status} | {duration_ms:.1f}ms | {details}")

def log_trading_decision(symbol, action, confidence, reasoning):
    """Log trading decisions for audit trail."""
    QUANTUM_LOGGER.info(f"TRADING | {symbol} | {action} | {confidence}% | {reasoning}")

def log_error_with_context(function_name, error, context=""):
    """Log errors with full context for debugging."""
    QUANTUM_LOGGER.error(f"ERROR | {function_name} | {str(error)} | Context: {context}")
    try:
        push_toast(f"{function_name}: {str(error)}", level="error")
    except Exception:
        pass

def log_system_health(component, status, metrics=None):
    """Log system health metrics."""
    metrics_str = f" | Metrics: {metrics}" if metrics else ""
    QUANTUM_LOGGER.info(f"HEALTH | {component} | {status}{metrics_str}")

def push_toast(message: str, level: str = "info"):
    """Push a transient UI toast message into STATE for Dash to render."""
    try:
        ts = time.time()
        last = STATE.get("last_toast") or {}
        # Avoid spamming same message repeatedly within 1s
        if last and last.get("msg") == message and (ts - float(last.get("ts", 0))) < 1.0:
            return
        STATE["last_toast"] = {"msg": str(message)[:300], "level": str(level), "ts": ts}
    except Exception:
        pass

def health_record(name: str, duration_ms: float):
    """Record last and EMA avg latency into STATE.system_health."""
    try:
        sh = STATE.get("system_health", {})
        sh[f"{name}_ms_last"] = float(duration_ms)
        prev = sh.get(f"{name}_ms_avg")
        sh[f"{name}_ms_avg"] = float(duration_ms) if prev is None else (0.8*float(prev) + 0.2*float(duration_ms))
...
def monitor_ai_function(func):
    """Decorator to monitor AI function performance."""
    @wraps(func)
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            log_ai_performance(func_name, True, duration_ms, f"Args: {len(args)} kwargs: {len(kwargs)}")
            return result
        except Exception as e:
            duration_ms = (time.time() - start_time) * 1000
            log_ai_performance(func_name, False, duration_ms, str(e))
            log_error_with_context(func_name, e, f"Args: {args[:2]}...")  # First 2 args only for privacy
            raise
    return wrapper
...
LOCAL_TZ = ZoneInfo("Asia/Jakarta")  # WIB Indonesia untuk Bitget exchange
...
# Professional Dashboard Layout - Responsive with Collapsible Sidebar
DASHBOARD_LAYOUT = {
    "display": "flex",
    "minHeight": "100vh",
...
ENTRY_CONF      = int(os.getenv("ENTRY_CONF", "75"))          # min confidence % - raised for 80% win rate target
...
ADX_MIN = float(os.getenv("ADX_MIN","25"))                    # min trend strength - raised for better accuracy
SQUEEZE_MIN_BBWIDTH = float(os.getenv("SQUEEZE_MIN_BBWIDTH","0.005"))  # skip when BB squeeze is too tight
P_UP_MIN_BUY = float(os.getenv("P_UP_MIN_BUY","0.65"))        # min up-probability for BUY - raised for better selectivity
P_UP_MAX_SELL = float(os.getenv("P_UP_MAX_SELL","0.35"))      # max up-probability for SELL - lowered for better selectivity
...
class QuantumStateManager:
    """Professional state management for QUANTUM CAPITAL trading platform."""
    
    def __init__(self):
        self.state_file = os.path.join(os.path.dirname(__file__), "state", "quantum_state.json")
        self._state = {
            "pos": None,
    ...
    def _load_state(self):
        ...
        try:
            ...
            if os.path.exists(self.state_file):
                ...
                QUANTUM_LOGGER.info(f"State loaded successfully from {self.state_file}")
        except Exception as e:
            QUANTUM_LOGGER.warning(f"Failed to load state: {e}. Using default state.")
    ...
    def _save_state(self):
        ...
        try:
            ...
            # Create a serializable copy of state
            serializable_state = {}
            for key, value in self._state.items():
                if key in ['ai_cache', 'fig_cache', 'ob_cache']:
                    # Skip non-serializable caches
                    continue
                elif key == 'pos' and value is not None:
                    # Ensure position data is serializable
                    serializable_state[key] = dict(value) if hasattr(value, 'items') else value
            ...
            QUANTUM_LOGGER.debug(f"State saved to {self.state_file}")
        except Exception as e:
            QUANTUM_LOGGER.error(f"Failed to save state: {e}")
    ...
    def set(self, key, value):
        """Set state value and trigger save."""
        self._state[key] = value
        self._save_state()
    ...
    # Dict-like helpers to be API-compatible with existing code paths
    def setdefault(self, key, default=None):
        """Return existing value for key, or set to default and return it.
        Mirrors dict.setdefault so callers like STATE.setdefault(...) work.
        Note: keys like 'fig_cache' / 'ob_cache' are excluded from persistence
        by _save_state(), so this is safe for cache containers.
        """
        if key not in self._state or self._state.get(key) is None:
            self._state[key] = default
            # persist only if serializable keys; _save_state handles filtering
            self._save_state()
        return self._state.get(key)

    def pop(self, key, default=None):
        """Pop a key from state similar to dict.pop()."""
        if key in self._state:
            val = self._state.pop(key)
            self._save_state()
            return val
        return default
...
def validate_ai_accuracy():
    """
    Validate AI prediction accuracy to ensure 80% win rate target.
    Returns True if system is ready for live trading.
    """
    try:
        ps = perf_snapshot()
        total_trades = ps["trades"]
        win_rate = ps["winrate_pos"]
        
        # Require minimum trades for statistical significance
        if total_trades < 50:
            print(f"[AI VALIDATION] Need more trades for validation: {total_trades}/50")
            return False
            
        # Check if win rate meets 80% target
        if win_rate >= 80.0:
            print(f"[AI VALIDATION] ✅ WIN RATE TARGET ACHIEVED: {win_rate:.1f}% ({ps['wins']}/{total_trades} trades)")
            return True
        else:
            print(f"[AI VALIDATION] ❌ Win rate below target: {win_rate:.1f}% (target: 80%)")
            return False
...
def compose_top_metrics(symbol: str, tf: str, di: pd.DataFrame, pos: dict | None):
    # Balance
...
def bitget():
    ...
    # Load markets may hit network; keep wrapped so we can fall back cleanly
    ex.load_markets()
    return ex


class SafeExchange:
    """Minimal offline-safe stub to let the UI run without network.

    Provides the attributes/methods referenced in the dashboard with
    harmless no-op return values. Intended for preview/testing the UI,
    callbacks, and cache-backed data flows when exchange access is blocked.
    """
    def __init__(self):
        # Minimal USDT perpetual markets commonly used in the UI
        self.markets = {
            "BTC/USDT:USDT": {"symbol": "BTC/USDT:USDT", "type": "swap", "quote": "USDT", "linear": True, "id": "BTCUSDT_UMCBL"},
            "ETH/USDT:USDT": {"symbol": "ETH/USDT:USDT", "type": "swap", "quote": "USDT", "linear": True, "id": "ETHUSDT_UMCBL"},
            "SOL/USDT:USDT": {"symbol": "SOL/USDT:USDT", "type": "swap", "quote": "USDT", "linear": True, "id": "SOLUSDT_UMCBL"},
        }
    ...
    def parse_timeframe(self, tf: str) -> int:
        m = {"1m": 60, "3m": 180, "5m": 300, "15m": 900, "30m": 1800, "1h": 3600, "2h": 7200, "4h": 14400, "1d": 86400}
        return m.get((tf or "").lower(), 60)
    ...
    def market(self, symbol: str):
        # Return minimal structure; caller has fallback handling if this fails
        s = (symbol or "").upper()
        if s in self.markets:
            return self.markets[s]
        # fabricate a basic id
        base_id = s.replace(":USDT", "").replace("/", "")
        return {"symbol": s, "id": f"{base_id}_UMCBL"}

    # ---- public data (no network) ----
    def fetch_ohlcv(self, *_, **__):
        # No network: return empty so cache path handles data
        return []

    def fetch_ticker(self, *_, **__):
        # Provide minimal structure
        return {"last": None}

    def fetch_tickers(self, symbols=None):
        # Return empty dict; UI will fall back to static list
        return {}

    def fetch_positions(self, *_, **__):
        return []

    def fetch_balance(self, *_, **__):
        return {"USDT": {"free": 0.0}, "info": {}}

    # ---- trading ops (no-ops in offline) ----
    def set_leverage(self, *_, **__):
        return {"status": "ok"}
...
def init_exchange():
    try:
        ...
        QUANTUM_LOGGER.warning(f"Exchange init failed: {e}. Falling back to SafeExchange (offline)")
        return SafeExchange()
...
def analyze_news_pulse_with_ai(items):
    """Enhanced news analysis with comprehensive error handling and fallback mechanisms."""
    ...
    try:
        ...
        if not titles:
            print("[NEWS AI] No titles to analyze - using fallback")
            return fallback_result
            
        payload = {"titles": titles}
        
        # Enhanced system prompt with clearer instructions
        system_prompt = """You are a financial market analyst. Analyze the provided news headlines and return ONLY valid JSON with these exact keys:
        - politics_sent: number between -1.0 and 1.0 (negative=bearish, positive=bullish)
        - crypto_sent: number between -1.0 and 1.0 (crypto market sentiment)
        - rate_bias: one of "cut", "hold", "hike", or "uncertain"
        - rate_prob: number between 0.0 and 1.0 (probability of rate_bias scenario)
        - highlights: array of up to 5 most important crypto/market headlines as strings
        ...
        for attempt in range(3):
            try:
                ...
                print(f"[NEWS AI] JSON parse attempt {attempt + 1} failed: {e}")
                if attempt == 0:
        ...
        if obj is None:
            raise ValueError(f"Failed to parse JSON after 3 attempts. Raw response: {raw[:200]}...")
        
        # Validate and sanitize the response
        validated_result = {}
        
        # Validate politics_sent
        politics = obj.get("politics_sent")
        if isinstance(politics, (int, float)) and -1.0 <= politics <= 1.0:
            validated_result["politics_sent"] = float(politics)
        else:
            validated_result["politics_sent"] = fallback_result["politics_sent"]
            
        # Validate crypto_sent
        crypto = obj.get("crypto_sent")
        if isinstance(crypto, (int, float)) and -1.0 <= crypto <= 1.0:
            validated_result["crypto_sent"] = float(crypto)
        else:
            validated_result["crypto_sent"] = fallback_result["crypto_sent"]
            
        # Validate rate_bias
        rate_bias = obj.get("rate_bias")
        if rate_bias in ["cut", "hold", "hike", "uncertain"]:
            validated_result["rate_bias"] = rate_bias
        else:
            validated_result["rate_bias"] = fallback_result["rate_bias"]
            
        # Validate rate_prob
        rate_prob = obj.get("rate_prob")
        if isinstance(rate_prob, (int, float)) and 0.0 <= rate_prob <= 1.0:
            validated_result["rate_prob"] = float(rate_prob)
        else:
            validated_result["rate_prob"] = fallback_result["rate_prob"]
            
        # Validate highlights
        highlights = obj.get("highlights", [])
        if isinstance(highlights, list) and all(isinstance(h, str) for h in highlights):
            validated_result["highlights"] = highlights[:5]  # Limit to 5
        else:
            validated_result["highlights"] = fallback_result["highlights"]
        
        # Merge with fallback result to ensure all keys exist
        final_result = fallback_result.copy()
        final_result.update(validated_result)
        
        print(f"[NEWS AI] Successfully analyzed {len(titles)} headlines")
        return final_result
        
    except ImportError as e:
        print(f"[NEWS AI] Import error: {e} - using fallback")
        return fallback_result
    except json.JSONDecodeError as e:
        print(f"[NEWS AI] JSON decode error: {e} - using fallback")
        return fallback_result
    except Exception as e:
        print(f"[NEWS AI] Unexpected error: {e} - using fallback")
        return fallback_result
...
def compute_indicators(df: pd.DataFrame, symbol: str = "UNKNOWN") -> pd.DataFrame:
    ...
    # Memory optimization: use float32 consistently, but exclude datetime columns
    numeric_columns = ['open', 'high', 'low', 'close', 'volume']
    for col in numeric_columns:
        if col in d.columns:
            d[col] = d[col].astype(DF_FLOAT_DTYPE)
...
def build_optimized_chart(di: pd.DataFrame, pos: dict | None, ob_zones: list, symbol: str, tf: str, overlay: dict | None = None) -> go.Figure:
    ...
    _ov = {"vwap": True, "psar": True, "fib": True, "brkfake": True, "wm": True}
                    ...
                    for i in range(len(swings)-1):
                        try:
                            ...
                            if idx1 < len(vis) and idx2 < len(vis):
                                dt1_raw = vis["datetime"].iloc[min(idx1, len(vis)-1)]
                                dt2_raw = vis["datetime"].iloc[min(idx2, len(vis)-1)]
                                
                                # Convert to proper timezone
                                dt1 = safe_datetime_convert(dt1_raw, LOCAL_TZ)
                                dt2 = safe_datetime_convert(dt2_raw, LOCAL_TZ)
    ...
    # Branding watermark (bottom-left, subtle)
    if _ov.get("wm", True):
        try:
            fig.add_annotation(
                xref="paper", yref="paper", x=0.005, y=0.01,
                text="QUANTUM CAPITAL • AI FUTURES TRADING",
                showarrow=False,
                font=dict(color="rgba(180, 188, 208, 0.55)", size=10),
                align="left",
...
# Professional Risk Management Suite for QUANTUM CAPITAL
class QuantumRiskManager:
    """Institutional-grade risk management with multiple position sizing algorithms."""
    ...
    def kelly_criterion_size(self, win_rate: float, avg_win: float, avg_loss: float, 
                           bankroll: float, max_risk: float = 0.25) -> float:
        ...
        final_fraction = min(conservative_kelly, max_risk)
        return max(0.0, final_fraction) * bankroll
    
    def volatility_adjusted_size(self, symbol: str, di: pd.DataFrame, 
                               base_size: float, target_vol: float = 0.15) -> float:
        """Adjust position size based on asset volatility."""
        try:
            # Calculate recent volatility (20-day rolling)
            returns = di['close'].pct_change().dropna()
            current_vol = returns.tail(20).std() * np.sqrt(365)  # Annualized volatility
            ...
            return max(base_size * 0.1, min(base_size * 3.0, adjusted_size))
            
        except Exception as e:
            log_error_with_context("volatility_adjusted_size", e, f"symbol: {symbol}")
            return base_size
    
    def risk_parity_allocation(self, positions: dict, new_symbol: str, 
                             target_risk: float = 0.10) -> float:
        """Calculate position size using risk parity approach."""
        try:
            total_portfolio_value = sum(pos.get('notional', 0) for pos in positions.values())
            
            if total_portfolio_value <= 0:
                return target_risk  # First position gets full allocation
                
            # Calculate current risk contribution of existing positions
            existing_risk = sum(
                pos.get('notional', 0) * pos.get('volatility', 0.1) 
                for pos in positions.values()
            )
            
            # Allocate remaining risk budget to new position
            remaining_risk_budget = target_risk - existing_risk
            
            return max(0.01, remaining_risk_budget)  # Minimum 1% allocation
            
        except Exception as e:
            log_error_with_context("risk_parity_allocation", e)
            return target_risk * 0.5  # Conservative fallback

def choose_leverage(symbol: str, timeframe: str, di: pd.DataFrame) -> int:
    """Enhanced leverage selection with professional risk management."""
    ...
    # Enhanced volatility-based leverage calculation
    v = atr_percent(di)
    
    # Get current equity and drawdown status
    current_equity = fetch_usdt_futures_balance("equity")
    equity_peak = STATE.get("perf", {}).get("equity_peak", current_equity)
    
    if current_equity and equity_peak:
        current_drawdown = max(0, (equity_peak - current_equity) / equity_peak)
    else:
        current_drawdown = 0
    
    # Base volatility adjustment
    if v <= 0.002:      # Very low volatility
        base_frac = 1.0
    elif v <= 0.005:    # Low volatility  
        base_frac = 0.8
    elif v <= 0.01:     # Medium volatility
        base_frac = 0.6
    ...
    if current_drawdown > 0.10:        # 10%+ drawdown
        drawdown_multiplier = 0.5      # Reduce leverage by 50%
    elif current_drawdown > 0.05:      # 5%+ drawdown
        drawdown_multiplier = 0.75     # Reduce leverage by 25%
    else:
        drawdown_multiplier = 1.0      # No reduction
    
    # Calculate final leverage
    final_frac = base_frac * drawdown_multiplier
    dyn = int(max(1, min(maxlev, round(maxlev * final_frac))))
    
    # Log leverage decision for monitoring
    log_trading_decision(symbol, f"LEVERAGE_SELECTED_{dyn}", 
                        int(final_frac * 100), 
                        f"Vol: {v:.4f}, Drawdown: {current_drawdown:.2%}")
    
    return dyn

# Initialize risk manager
QUANTUM_RISK = QuantumRiskManager()

def compute_notional_usdt_enhanced(symbol: str, last_price: float, di: pd.DataFrame) -> float:
    """Enhanced notional calculation with professional risk management."""
    if FULL_MARGIN:
        fav = fetch_usdt_futures_balance("available")
        if fav and fav > 10:
            base_notional = float(fav) * MARGIN_USE_PCT
        else:
            base_notional = BASE_ORDER_USDT
    else:
        base_notional = BASE_ORDER_USDT
    ...
    try:
        ...
        base_notional = QUANTUM_RISK.volatility_adjusted_size(symbol, di, base_notional)
        
        # Get trading performance for Kelly Criterion
        perf = STATE.get("perf", {})
        total_trades = perf.get("trades", 0)
        wins = perf.get("wins", 0)
        losses = perf.get("losses", 0)
        
        if total_trades > 10:  # Only apply Kelly after sufficient trade history
            win_rate = wins / total_trades
            # Estimate average win/loss from recent performance
            avg_win = 0.02  # Rough estimate - could be enhanced with actual tracking
            avg_loss = -0.015
            
            current_equity = fetch_usdt_futures_balance("equity") or base_notional * 10
            kelly_size = QUANTUM_RISK.kelly_criterion_size(win_rate, avg_win, avg_loss, 
                                                         current_equity, max_risk=0.10)
            
            # Use Kelly sizing if it's reasonable
            if 0 < kelly_size < current_equity:
                base_notional = min(base_notional, kelly_size)
                
        # Final safety caps
        max_notional = fetch_usdt_futures_balance("equity") * 0.20 if fetch_usdt_futures_balance("equity") else base_notional * 2
        return min(base_notional, max_notional)
        
    except Exception as e:
        log_error_with_context("compute_notional_usdt_enhanced", e, f"symbol: {symbol}")
...
def ai_explain(symbol, timeframe, action, conf, reason, last_row):
    """Enhanced AI explanation with comprehensive error handling and fallback."""
    ...
    try:
        ...
        try:
            txt = (
                ...
                f"Price: {float(last_row.get('close', 0)):.6f}\n"
                f"EMA20: {float(last_row.get('ema20', 0)):.6f} | EMA50: {float(last_row.get('ema50', 0)):.6f}\n"
                f"MACD Histogram: {float(last_row.get('macd_hist', 0)):.6f}\n"
                f"RSI: {float(last_row.get('rsi', 50)):.1f} | Stoch K: {float(last_row.get('stoch_k', 50)):.1f}\n"
                f"Trading Decision: {action} | Confidence: {conf}%\n"
                f"Technical Reasoning: {reason}"
            )
        except (KeyError, TypeError, ValueError) as e:
            print(f"[AI EXPLAIN] Data formatting error: {e}")
            return fallback_explanation
        
        # Enhanced system prompt for institutional-grade analysis
        system_prompt = """You are QUANTUM CAPITAL's institutional AI trading analyst. Provide concise, professional market analysis in 2-3 sentences.

Requirements:
- Start with the trading action and confidence level
- Explain the key technical factors driving the decision  
- Include specific price levels or ratios when relevant
- Keep tone professional and institutional
...
Format: Professional paragraph format, no bullet points."""
        ...
        # Enhance with QUANTUM CAPITAL branding
        branded_response = f"🧠 QUANTUM CAPITAL AI Analysis:\n{ai_response}"
        
        print(f"[AI EXPLAIN] Successfully generated explanation for {symbol}")
        return branded_response
        
    except ImportError as e:
        print(f"[AI EXPLAIN] Import error: {e}")
        return fallback_explanation
    except Exception as e:
...
def create_sidebar_nav():
    ...
    return html.Div([
        # Fixed Header: Logo and Toggle Button (always visible)
        html.Div([
            # Logo Q (collapsible)
            html.Div("Q", id="logo-q", style={
                "width": "40px",
                "height": "40px",
                "borderRadius": "8px",
                "background": f"linear-gradient(45deg, {COLORS['accent']}, {COLORS['info']})",
        ...
        # Collapsible Content (logo text + trading controls)
        html.Div([
            ...
            # Trading controls (collapsible)
            create_trading_controls()
        ], id="sidebar-content", style={"display": "block"})
    ...
    return html.Div([
        ...
        html.Div([
            ...
            dcc.Dropdown(
                ...
                options=get_symbol_options(100),
                value="BTC/USDT:USDT",
                style={
        ...
        # System Health Panel
        html.Div([
            html.Label("SYSTEM HEALTH", style={
                "fontSize": "11px",
                "fontWeight": "600",
                "color": COLORS['text_secondary'],
                "marginBottom": "12px",
            ...
            html.Div(id="health-metrics", style={
                "whiteSpace": "pre-wrap",
            ...
            html.Div([
                html.Button(
                    ...
                    style={
                        ...
                        "padding": "12px 16px",
                        "marginBottom": "8px",
                        "background": f"linear-gradient(45deg, {COLORS['success']}, #00CC73)",
                        "color": "white",
                        "border": "none",
                        "borderRadius": "8px",
                        "fontSize": "12px",
                        ...
                        "cursor": "pointer",
                        "transition": "all 0.3s ease",
                        "boxShadow": f"0 4px 12px rgba(0, 255, 136, 0.3)",
                        "display": "block"
                    }
                ),
                html.Button(
                    ...
                    style={
                        "width": "100%",
                        "padding": "12px 16px",
                        "background": f"linear-gradient(45deg, {COLORS['danger']}, #CC2950)",
                        "color": "white",
                        "border": "none",
                        "borderRadius": "8px",
                        "fontSize": "12px",
                        ...
                        "cursor": "pointer",
                        "transition": "all 0.3s ease",
                        "boxShadow": f"0 4px 12px rgba(255, 51, 102, 0.3)",
                        "display": "none"
                    }
                )
    ])
...
def get_symbol_options(limit=60):
    ...
    try:
        ...
        rows = []
        for s,t in (tick or {}).items():
            vol = (t or {}).get("quoteVolume") or (t or {}).get("baseVolume") or 0
            rows.append((s, float(vol or 0)))
        # Fallback to original syms if no volume rows available (e.g., offline stub)
        if rows:
...
app.layout = html.Div([
    ...
    # Custom CSS will be injected via external_stylesheets in app initialization
    ...
    html.Div([
        ...
        # Main content wrapper
        html.Div([
            # Header Bar (Brand + Connection Status)
            html.Div(
                create_logo_header(),
                style=HEADER_STYLE
            ),
                        ...
                        html.Div([
                            dcc.Checklist(
                                ...
                                options=[
                                    ...
                                    {"label": "WATERMARK", "value": "wm"}
                                ],
                                value=["vwap", "psar", "fib", "ew", "brkfake", "wm"],
                                style={
                                    "color": COLORS['text_primary'],
                                    "fontSize": "12px",
                                    "fontWeight": "500"
        ...
        ], style=MAIN_CONTENT_STYLE),  # Close main content wrapper
        ...
        html.Div([
            ...
            html.Div(id="export-done", style={"display":"none"}),
            dcc.Download(id="download-csv")
        ])
    ], style=DASHBOARD_LAYOUT)
])

app.config.suppress_callback_exceptions = True

# Clientside callback to update metric cards from hidden divs
app.clientside_callback(
    """
    function(price, bal, sig, aisig, pos, rr, progress, ai_target) {
        function updateCard(cardIndex, value) {
            const cards = document.querySelectorAll('.professional-grid > div');
            if (cards[cardIndex]) {
        ...
        // Update each card based on the order in the grid
        updateCard(0, price);           // Current Price
        updateCard(1, bal);             // USDT Balance  
        updateCard(2, sig);             // Strategy Signal
        updateCard(3, aisig);           // AI Confidence
        updateCard(4, pos);             // Position
        updateCard(5, rr);              // Risk:Reward
        updateCard(6, progress);        // Progress
        updateCard(7, ai_target);       // AI Target
        
        return window.dash_clientside.no_update;
    }
    """,
    Output("professional-grid", "id"),  # Dummy output
    [Input("price", "children"),
     Input("bal", "children"), 
     Input("sig", "children"),
     Input("aisig", "children"),
    Input("pos", "children"),
    Input("rr", "children"), 
    Input("progress", "children"),
    Input("ai-target", "children")]
)

# Client-side export to PNG with branding
app.clientside_callback(
    """
    function(n) {
        if (!n || n < 1) { return window.dash_clientside.no_update; }
        var gd = document.getElementById('chart');
        if (gd) {
            try {
                Plotly.downloadImage(gd, {
                    format: 'png', filename: 'quantum_capital_chart', height: 900, width: 1400, scale: 2
                });
            } catch (e) { /* ignore */ }
        }
        return 'done';
    ...
    """,
    Output("export-done", "children"),
    Input("export-png", "n_clicks"),
    prevent_initial_call=True
)

# Client-side signal alert banner
app.clientside_callback(
    """
    function(sig) {
        let action = '';
        if (sig && typeof sig === 'string') {
            action = sig.trim().split(' ')[0].toUpperCase();
        }
        ...
        if (action === 'BUY') {
            ...
            if (el) {
                el.style.display = 'inline-block';
                el.style.background = 'rgba(255,51,102,0.12)';
    ...
    Output("signal-alert", "style"),
    Input("sig", "children")
)

# Server-side CSV export of current dataset with indicators
@app.callback(
...
def export_csv(n_clicks, symbol, tf):
    try:
        if not n_clicks or not symbol or not tf:
            return no_update
        df, status = ensure_clean_tf_df(symbol, tf)
        if df is None or df.empty:
            return no_update
        vis = compute_indicators(df, symbol)
        # Ensure plain types for CSV
        out = vis.copy()
        if 'datetime' in out.columns:
            try:
                dtser = pd.to_datetime(out['datetime'], errors='coerce', utc=True)
                out['datetime'] = dtser.dt.tz_convert(None)
            except Exception:
                pass
        fname = f"quantum_capital_{symbol.replace('/', '_').replace(':', '_')}_{tf}.csv"
        return dcc.send_data_frame(out.to_csv, fname, index=False)
    except Exception as e:
        log_error_with_context('export_csv', e, f"symbol={symbol}, tf={tf}")
        return no_update
...
def update_health(_n):
    try:
        sh = STATE.get("system_health", {})
        conn = sh.get("connection_status", "unknown")
        last_ts = sh.get("last_update") or 0
        last_txt = datetime.fromtimestamp(last_ts).strftime('%H:%M:%S') if last_ts else "—"
        api_err = int(sh.get("api_errors", 0))
        bg = bool(STATE.get("bg_alive"))
        news = bool(STATE.get("news_alive"))
        cache_len = len(CACHE or {})
        figc = len(STATE.get("fig_cache", {}) or {})
        ...
        lines = [
            ...
            *([f"Balance ms: last={sh.get('balance_ms_last', '—'):.0f} avg={sh.get('balance_ms_avg', '—'):.0f}"] if isinstance(sh.get('balance_ms_last'), (int,float)) else []),
            *([f"Positions ms: last={sh.get('positions_ms_last', '—'):.0f} avg={sh.get('positions_ms_avg', '—'):.0f}"] if isinstance(sh.get('positions_ms_last'), (int,float)) else []),
            *([f"Orders ms: last={sh.get('orders_ms_last', '—'):.0f} avg={sh.get('orders_ms_avg', '—'):.0f}"] if isinstance(sh.get('orders_ms_last'), (int,float)) else []),
        ]
        return "\n".join(lines)
    except Exception as e:
        log_error_with_context('update_health', e)
        return "—"
...
def toggle_sidebar(n_clicks):
    ...
    if collapsed:
        # Collapsed sidebar style - hide logo Q and content
        sidebar_style = {
            **SIDEBAR_STYLE,
            "width": "70px",
            "minWidth": "70px",
        ...
        logo_style = {"display": "none"}  # Hide logo Q when collapsed
    else:
        # Expanded sidebar style - show everything
    ...
    return sidebar_style, content_style, logo_style
...
def refresh_dashboard(n_intervals, timer_disabled, symbol, tf, start_clicks, stop_clicks, 
    ...
    try:
        ...
        try:
            sh = STATE.get("system_health", {})
            sh["last_update"] = time.time()
            sh["connection_status"] = "online" if type(EX).__name__ != "SafeExchange" else "offline"
            STATE["system_health"] = sh
        except Exception:
            pass
        ...
        # Update position with live exchange data if needed
        ...
        if not norm:
            ...
            return (pos_store or STATE.get("pos")), "--", "—", "—", "—", [], fig, "Symbol tidak dikenali. Pilih dari dropdown.", "—", "—", "—", "—", "—"
        symbol = norm
        np.seterr(all='ignore')
        running = not bool(timer_disabled)
        # arahkan background fetcher dan pastikan aktif
        ...
        if df.empty:
            ...
            return (pos_store or STATE.get("pos")), "--", "—", "—", "—", [], fig, "Memuat data…", "—", "—", "—", "—", "—"
        ...
        # Plotly x (datetime to WIB) - use robust timezone conversion
        xdt = safe_timezone_convert(vis["datetime"], LOCAL_TZ)
        ...
        try:
            ...
            for z in ob_vis:
                ...
                z_x = safe_datetime_convert(z_ts, LOCAL_TZ)
        ...
        tail = di.tail(50)
        tail_xdt = safe_timezone_convert(tail["datetime"], LOCAL_TZ)
        def add_marks(mask, text):
            mk = tail[mask].copy()
            if mk.empty: return
            mk_xdt = safe_timezone_convert(mk["datetime"], LOCAL_TZ)
            fig.add_trace(go.Scatter(
        ...
        return pos, price_txt, bal_txt, sig_txt, ai_txt, cards, fig, rec, pos_txt, rr_txt, progress_txt, ai_target_txt, top_md
    
    except Exception as e:
        import traceback
        print(f"[REFRESH] CRITICAL ERROR: {e}")
        print(f"[REFRESH] Full traceback:\n{traceback.format_exc()}")
        print(f"[REFRESH] Inputs: symbol={symbol}, tf={tf}, timer_disabled={timer_disabled}")
        try:
            sh = STATE.get("system_health", {})
            sh["api_errors"] = int(sh.get("api_errors", 0)) + 1
            sh["last_update"] = time.time()
            STATE["system_health"] = sh
        except Exception:
            pass
        
        # Fallback return untuk mencegah None
        empty_fig = go.Figure()
        empty_fig.add_annotation(text=f"Error: {str(e)}", x=0.5, y=0.5, xref="paper", yref="paper", showarrow=False)
        return (pos_store or STATE.get("pos", {})), "ERROR", "ERROR", "ERROR", "ERROR", [], empty_fig, "ERROR", "ERROR", "ERROR", "ERROR", "ERROR", "ERROR"


def safe_timezone_convert(*args, **kwargs):
    """
    QUANTUM CAPITAL: Bulletproof timezone converter for WIB Indonesia trading.
    Handles runtime argument expansion and Dash callback context issues.
    
    Args:
        datetime_series: pandas datetime series  
        target_tz: target timezone (default: Asia/Jakarta for WIB)
        
    Returns:
        pandas datetime series in WIB timezone (timezone-naive for Plotly)
    """
    try:
        # RUNTIME ARGUMENT VALIDATION - Fix for "9 arguments" error
        if len(args) == 0:
            raise ValueError("safe_timezone_convert requires at least 1 argument (datetime_series)")
        elif len(args) == 1:
            datetime_series = args[0]
            target_tz = kwargs.get('target_tz', "Asia/Jakarta")
        elif len(args) == 2:
            datetime_series, target_tz = args[0], args[1]
        elif len(args) > 2:
            # CRITICAL FIX: Handle runtime argument expansion
            QUANTUM_LOGGER.warning(f"safe_timezone_convert received {len(args)} args, expected 2. Using first 2.")
            QUANTUM_LOGGER.debug(f"Arguments received: {[type(arg) for arg in args]}")
            datetime_series, target_tz = args[0], args[1]
            # Log the unexpected extra arguments for debugging
            extra_args = args[2:]
            QUANTUM_LOGGER.debug(f"Ignoring extra arguments: {[str(arg)[:50] for arg in extra_args]}")
        else:
            datetime_series = args[0]
        ...
        if datetime_series is None:
            QUANTUM_LOGGER.warning("datetime_series is None, returning empty series")
            return pd.Series([], dtype='datetime64[ns]')
            
        # Handle empty or invalid series
        if hasattr(datetime_series, '__len__') and len(datetime_series) == 0:
            return pd.Series([], dtype='datetime64[ns]')
        
        # Convert single values to series if needed
        if not hasattr(datetime_series, 'dtype'):
            datetime_series = pd.Series([datetime_series])
        
        # Handle both Series and DatetimeIndex
        is_series = isinstance(datetime_series, pd.Series)
        is_index = isinstance(datetime_series, pd.DatetimeIndex)
        
        # Remove NaT values for clean processing
        if datetime_series.isna().any():
            QUANTUM_LOGGER.warning(f"Found {datetime_series.isna().sum()} NaT values in datetime conversion")
            datetime_series = datetime_series.dropna()
        
        # Fast path for WIB Indonesia (UTC+7)
        target_tz_obj = ZoneInfo("Asia/Jakarta") if isinstance(target_tz, str) else target_tz
        
        # Case 1: Already timezone-aware - direct conversion to WIB
        if hasattr(datetime_series.dtype, 'tz') and datetime_series.dtype.tz is not None:
            try:
                if is_series:
                    converted = datetime_series.dt.tz_convert(target_tz_obj).dt.tz_localize(None)
                else:  # DatetimeIndex
                    converted = datetime_series.tz_convert(target_tz_obj).tz_localize(None)
                QUANTUM_LOGGER.debug(f"Timezone conversion: {len(converted)} timestamps to WIB")
                return converted
            except Exception as e:
                QUANTUM_LOGGER.warning(f"Direct tz_convert to WIB failed: {e}")
        
        # Case 2: Timezone-naive UTC → WIB conversion (UTC+7)
        try:
            if is_series:
                utc_localized = datetime_series.dt.tz_localize("UTC")
                wib_converted = utc_localized.dt.tz_convert(target_tz_obj)
                wib_naive = wib_converted.dt.tz_localize(None)
            else:  # DatetimeIndex
                utc_localized = datetime_series.tz_localize("UTC")
                wib_converted = utc_localized.tz_convert(target_tz_obj)
                wib_naive = wib_converted.tz_localize(None)
            QUANTUM_LOGGER.debug(f"UTC→WIB conversion: {len(wib_naive)} timestamps")
            return wib_naive
        except Exception as e:
            QUANTUM_LOGGER.warning(f"UTC→WIB conversion failed: {e}")
        
        # Case 3: Simple offset approach for WIB (+7 hours from UTC)
        try:
            # Add 7 hours for WIB timezone (UTC+7)
            wib_offset = datetime_series + pd.Timedelta(hours=7)
            QUANTUM_LOGGER.debug(f"Simple WIB offset applied to {len(wib_offset)} timestamps")
            return wib_offset
        except Exception as e:
            QUANTUM_LOGGER.warning(f"Simple WIB offset failed: {e}")
            
        # Case 4: Return original series as fallback
        QUANTUM_LOGGER.warning("All WIB timezone conversions failed, returning original series")
        return datetime_series
        
    except Exception as e:
        QUANTUM_LOGGER.error(f"Critical error in safe_timezone_convert: {e}")
        # Emergency fallback - return empty series if original is corrupted
        try:
            return datetime_series if datetime_series is not None else pd.Series([], dtype='datetime64[ns]')
        except:
            return pd.Series([], dtype='datetime64[ns]')
